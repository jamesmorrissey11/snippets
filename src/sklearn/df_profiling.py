...
def log_columns(path_to_csv, file_name):
...
def feature_skewness(df, cols):
    print(df[cols].skew().to_frame().rename(columns={0: "Feature Skewness"}))


def clean_categorical_field(df, column, replace_value=None):
    if replace_value:
        df[column] = df[column].replace(replace_value, np.nan)
        print(f"Replaced {replace_value} with np.nan")
